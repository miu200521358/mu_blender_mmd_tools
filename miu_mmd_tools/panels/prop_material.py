...
class MMDMaterialPanel(Panel):
    bl_idname = 'MATERIAL_PT_miu_mmd_tools_material'
...
class MMDTexturePanel(Panel):
    bl_idname = 'MATERIAL_PT_miu_mmd_tools_texture'
    ...
    def draw(self, context):
        ...
        if tex:
            if tex.type == 'IMAGE' and tex.image:
                ...
                r.operator('miu_mmd_tools.material_remove_texture', text='', icon='PANEL_CLOSE')
            else:
                r.operator('miu_mmd_tools.material_remove_texture', text='Remove', icon='PANEL_CLOSE')
                r.label(icon='ERROR')
        else:
            r.operator('miu_mmd_tools.material_open_texture', text='Add', icon=ICON_FILE_FOLDER)
        ...
        if tex:
            if tex.type == 'IMAGE' and tex.image:
                r.prop(tex.image, 'filepath', text='')
                r.operator('miu_mmd_tools.material_remove_sphere_texture', text='', icon='PANEL_CLOSE')
            else:
                r.operator('miu_mmd_tools.material_remove_sphere_texture', text='Remove', icon='PANEL_CLOSE')
                r.label(icon='ERROR')
        else:
            r.operator('miu_mmd_tools.material_open_sphere_texture', text='Add', icon=ICON_FILE_FOLDER)
