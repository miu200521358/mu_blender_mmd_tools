...
from miu_mmd_tools.core.sdef import FnSDEF
...
class MMDModelObjectDisplayPanel(_PanelBase, Panel):
    bl_idname = 'OBJECT_PT_miu_mmd_tools_root_object_display'
...
class MMDViewPanel(_PanelBase, Panel):
    bl_idname = 'OBJECT_PT_miu_mmd_tools_view'
    ...
    def draw(self, context):
        ...
        r = c.row(align=True)
        r.operator('miu_mmd_tools.set_glsl_shading', text='GLSL')
        r.operator('miu_mmd_tools.set_shadeless_glsl_shading', text='Shadeless')
        r = c.row(align=True)
        r.operator('miu_mmd_tools.reset_shading', text='Reset')

@register_wrap
class MMDSDEFPanel(_PanelBase, Panel):
    bl_idname = 'OBJECT_PT_miu_mmd_tools_sdef'
    bl_label = 'MMD SDEF Driver'

    def draw(self, context):
        c = self.layout.column(align=True)
        c.operator('miu_mmd_tools.sdef_bind', text='Bind')
        c.operator('miu_mmd_tools.sdef_unbind', text='Unbind')
        row = c.row()
        row.label(text='Cache Info: %d data'%(len(FnSDEF.g_verts)), icon='INFO')
        row.operator('miu_mmd_tools.sdef_cache_reset', text='', icon='X')
