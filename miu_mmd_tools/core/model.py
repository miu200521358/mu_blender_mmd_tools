...
from miu_mmd_tools.core import rigid_body
...
class Model:
    ...
    def clean(self):
        ...
        for i in pose_bones:
            if 'miu_mmd_tools_rigid_track' in i.constraints:
                const = i.constraints['miu_mmd_tools_rigid_track']
                i.constraints.remove(const)

        rigid_track_counts = 0
        for i in self.rigidBodies():
            rigid_type = int(i.mmd_rigid.type)
            if 'miu_mmd_tools_rigid_parent' not in i.constraints:
            ...
            relation = i.constraints['miu_mmd_tools_rigid_parent']
            relation.mute = True
    ...
    def __postBuild(self):
        ...
        if arm:
            for p_bone in arm.pose.bones:
                c = p_bone.constraints.get('miu_mmd_tools_rigid_track', None)
                if c:
                    c.mute = False
    ...
    def updateRigid(self, rigid_obj):
        ...
        relation = rigid_obj.constraints['miu_mmd_tools_rigid_parent']
        arm = relation.target
        bone_name = relation.subtarget
        ...
        if arm is not None and bone_name != '':
            ...
            if rigid_type == rigid_body.MODE_STATIC:
                ...
                if 'miu_mmd_tools_rigid_track' not in target_bone.constraints:
                    ...
                    const.mute = True
                    const.name='miu_mmd_tools_rigid_track'
                    const.target = empty
                else:
                    empty = target_bone.constraints['miu_mmd_tools_rigid_track'].target
    ...
    def __makeSpring(self, target, base_obj, spring_stiffness):
        ...
        t = spring_target.constraints.get('miu_mmd_tools_rigid_parent')
        if t is not None:
